...
public class SVDandFT {
	...
	public static void invertSVD(SimpleMatrix A)
	{
		...
		SinvDiag = svd.getS().getDiag();
		
		for(int i = 0; i<SinvDiag.getLen(); i++)
			SinvDiag.setElementValue(i, 1/SinvDiag.getElement(i));
		...
		{
		int condMin;
		for(condMin = 0; condMin< Sinv.getDiag().getLen(); condMin++) {
			if(SinvDiag.getElement(condMin) == 0)
				 break;
		}
				
		
		double cond =  Sinv.getDiag().max() / Sinv.getDiag().getElement(--condMin);
		//oder
		cond = svd.cond();
	...
	public static void optimizationProblem1(SimpleMatrix A, int rankDeficiency)
	{
		...
		//Slowrank.setElementValue(rank, rank, 0);
	...
	public static void optimizationProblem3(Grid2D image, int rank)
	{
		...
		SimpleMatrix usK = SimpleOperators.multiplyMatrixProd(svd.getU().getSubMatrix(0, 0, svd.getU().getRows(), rank), svd.getU().getSubMatrix(0,0,rank, rank));
	...
	public static void main(String[] args) {
		...
		String filename = "/proj/i5dmip/yannicjeremy/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/yu_fill.jpg";
		...
		String filenameShepp = "/proj/i5dmip/yannicjeremy/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/Shepp-Logan Phantom.tif";
